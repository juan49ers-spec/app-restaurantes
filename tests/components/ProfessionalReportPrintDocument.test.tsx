...
describe('ProfessionalReportPrintDocument', () => {
  it('renders a branded printable report with index and data-quality appendix', () => {
    ...
    expect(screen.getAllByText('Casa Juan').length).toBeGreaterThan(0)
    expect(screen.getAllByText('ControlHub Consulting').length).toBeGreaterThanOrEqual(2)
    expect(screen.getAllByText(/consultor@controlhub.es/).length).toBeGreaterThanOrEqual(1)
    expect(screen.getAllByText(/Casa Juan · 2026-02-01 a 2026-02-28 · v3/).length).toBeGreaterThan(0)
    expect(screen.getByText('Ficha de control')).toBeInTheDocument()
    expect(screen.getByText('Publicado')).toBeInTheDocument()
    expect(screen.getByText('Exportado')).toBeInTheDocument()
    expect(screen.getByText('No registrado')).toBeInTheDocument()
    expect(screen.getAllByText('Confianza').length).toBeGreaterThanOrEqual(1)
    ...
    expect(screen.getByText('Sin incidencias globales críticas en el snapshot guardado.')).toBeInTheDocument()
