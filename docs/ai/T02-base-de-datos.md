...
| `consultant_restaurants` | `consultant_user_id`, `restaurant_id`, `role`, `status` | Relación explícita consultor-restaurante para cartera multi-cliente. `status='ACTIVE'` permite seleccionar el restaurante en `/consultant`; `PAUSED`/`REVOKED` no dan acceso operativo. |
