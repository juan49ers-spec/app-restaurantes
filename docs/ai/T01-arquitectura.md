...
- `/api/health` es el endpoint público mínimo de salud para producción. El proxy lo deja pasar sin sesión, comprueba conectividad con Supabase, devuelve `200`/`503`, usa `Cache-Control: no-store` y no expone variables de entorno ni secretos.
