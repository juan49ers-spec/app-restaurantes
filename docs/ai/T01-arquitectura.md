...
- Playwright usa por defecto `http://127.0.0.1:3100` para evitar colisiones con servidores locales en `:3000`. Se puede cambiar con `E2E_PORT` o `BASE_URL`; si `BASE_URL` apunta a una URL remota, Playwright no arranca un servidor local.
- `npm run qa:client-flow` carga `.env.local` y `.env` antes de lanzar las comprobaciones. Las credenciales E2E (`E2E_EMAIL`, `E2E_PASSWORD`) deben vivir fuera del código; los tests visuales se saltan con explicación si no están configuradas.
