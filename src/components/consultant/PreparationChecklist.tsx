...
const CHECKLIST_GROUPS: Array<{
...
}> = [
  {
    severity: 'blocker',
    label: 'Bloqueantes',
    description: 'Puntos que deben quedar resueltos antes de entregar un informe fiable.',
  },
  {
    severity: 'warning',
    label: 'Recomendados',
    description: 'Mejoras que elevan la calidad de la entrega sin bloquear el avance.',
  },
  {
    severity: 'info',
    label: 'Listos',
    description: 'Puntos ya cubiertos para el periodo seleccionado.',
  },
]
...
export function PreparationChecklist({ initialChecklist }: PreparationChecklistProps) {
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className={cn('mt-5 space-y-5', isPending && 'opacity-60')}>
        {CHECKLIST_GROUPS.map(group => {
          const groupItems = checklist.items.filter(item => item.severity === group.severity)
          if (groupItems.length === 0) return null

          const headingId = `preparation-checklist-${group.severity}`

          return (
            <section key={group.severity} role="region" aria-labelledby={headingId}>
              <div className="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <h3 id={headingId} className="text-sm font-semibold text-slate-950">{group.label}</h3>
                  <p className="text-xs leading-5 text-slate-500">{group.description}</p>
                </div>
                <Badge variant="outline" className="w-fit rounded-md border-slate-200 bg-white text-slate-600">
                  {groupItems.length} puntos
                </Badge>
              </div>
